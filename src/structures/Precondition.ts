...
import { Precondition as SapphirePrecondition, PreconditionOptions, PieceContext } from "@sapphire/framework";
...
abstract class Precondition extends SapphirePrecondition {
    ...
    public onUnload() {
        if (this.shouldRun) this.logger.loader(`Successfully unloaded precondition ${title(this.name)}!`);
